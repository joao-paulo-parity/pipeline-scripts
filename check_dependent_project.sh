...
patch_and_check_dependent() {
  ...
  for comp in "${dependent_companions[@]}"; do
    echo "Patching $this_repo into the $comp companion, which is a dependency of $dependent_repo, assuming $comp depends on $this_repo. Reasoning: if a companion was referenced in this PR or a companion of this PR, then it probably is has a dependency on this PR, since PR descriptions are processed starting from the dependencies."
    diener patch \
